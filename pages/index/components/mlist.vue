<template name="myList">
	<view class="list">
		<view class="flex justify-between padding-bottom-sm margin-sm" v-for="(item,index) in takeshop" :key="index" :data-data="item"
		 @click="takeOut(item.openid)">
			<view class style="width:160rpx;height:160rpx;overflow: hidden;position: relative;">
				...
				<image style="width:160rpx;height:160rpx;border-radius:8rpx; display: block;" mode="scaleToFill" lazy-load="true" :src="item.logo" />
			</view>
			<view style="width:510rpx">
				<view class="padding-bottom-xs text-black text-bold text-xl">{{item.shop}}</view>
				...
				<view class="padding-bottom-xs margin-top-xs solid-bottom text-sm">
					...
					<text class="text-price text-sm">{{item.delivering}}</text>
					<text class="margin-left-sm text-sm">配送</text>
					<text class="text-price text-sm">{{item.physical}}</text>
					<text class="margin-left-sm text-sm">人均</text>
					<text class="text-price text-sm">{{item.capita}}</text>
				</view>
			</view>
		</view>
