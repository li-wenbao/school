		...
		<view class="flex justify-between padding-bottom-sm margin-sm" v-for="(item,index) in storeList" :key="index" :data-data="item"
		 @click="hanlderThe">
			<view class style="width:160rpx;height:160rpx;overflow: hidden;position: relative;">
				...
				<image style="width:160rpx;height:160rpx;border-radius:8rpx; display: block;" mode="scaleToFill" lazy-load="true" :src="item.logoId" />
			</view>
			<view style="width:510rpx">
				<view class="padding-bottom-xs text-black text-bold text-xl">{{item.typeName}}</view>
